...
#     send_to_admin(f'autolog called at {dtnow}')
#     #get every username in the database and pass it to log_macros to log the info for each user
...
#
# def autolog_scheduler():
#     # Schedule the autolog function to run at 2359 every day
#     schedule.every().day.at("13:07").do(autolog, time=datetime.datetime.now().strftime('"%m/%d/%Y, %H:%M:%S.%f"') )
#     now = datetime.datetime.now()
#     print(f'autolog_scheduler called at {now}')
#     send_to_admin(f'autolog_scheduler called at {now}') #send to admin to let them know autologscheduler called
#
#     while True:
#
#         # Check if it's time to run the scheduled function
...
async def viewgymhandle(update: Update, context):
    ...
    names = cur.execute(f'SELECT * FROM uniquedata WHERE name LIKE \'%lift%\' AND name LIKE \'%{group}%\'').fetchall()
...
async def select_option(update: Update, context):
    ...
    if len(splitsearchname) == 1:
        sql1 = f'SELECT * FROM uniquedata WHERE name LIKE \'%{a}%\' AND name LIKE \'%{searchname}%\''
    elif len(splitsearchname) == 2:
        sql1 = f'SELECT * FROM uniquedata WHERE name LIKE \'%{a}%\' AND name LIKE \'%{splitsearchname[0]}%\' AND name LIKE \'%{splitsearchname[1]}%\''
    elif len(splitsearchname) == 3:
        sql1 = f'SELECT * FROM uniquedata WHERE name LIKE \'%{a}%\' AND name LIKE \'%{splitsearchname[0]}%\' AND name LIKE \'%{splitsearchname[1]}%\' AND name LIKE \'%{splitsearchname[2]}%\''
    elif len(splitsearchname) > 3:
        x = ' '.join(splitsearchname[2:])
        sql1 = f'SELECT * FROM uniquedata WHERE name LIKE \'%{a}%\' AND name LIKE \'%{splitsearchname[0]}%\' AND name LIKE \'%{splitsearchname[1]}%\' AND name LIKE \'%{x}%\''
    print(sql1)
    try:
        results = cur.execute(sql1).fetchall()
        if len(results) < 1:
...
async def req_for_quantity(update: Update, context):
    ...
    if a == 'food':
        ...
        userdatadb = estab_conn('userdata')
        cur = userdatadb.cursor()
        x = 1
        while x > 0:
            ...
            w = cur.execute(sql2).fetchall()[-1][0]
            if 10.0 < w < 200.0:
                weight = w
                x = 0
            else:
                x += 1 #get most recent weight until reach default weight
...
{quantity} minutes, {calorie} cals'''
    ...
    username = verify_if_account_exists(userid, context, prin=False) #get username from viae and prevents message send
    try:
        conn = estab_conn(username)
        cur = conn.cursor()
        sql = f'INSERT INTO activitylog (dtg, subtype, title, calorie, description, cost, printstring) VALUES(?,?,?,?,?,?,?)'
        cur.execute(sql, (sql_dtg, sql_subtype, sql_title, sql_calorie, sql_description, sql_cost, sql_printstring))
        conn.commit()
        conn.close()
...
async def save_entry(update: Update, context):
    ...
    try:
        conn = estab_conn(username)
        cur = conn.cursor()
        getlastrowidsql = 'SELECT id from uniquedata'
        lastid = cur.execute(getlastrowidsql).fetchall()[-1][0]
        sql_id = int(lastid) + 1

        sql = f'INSERT INTO uniquedata (name, quantity, quantityunit, calorie, cpf) VALUES(?,?,?,?,?)'
        cur.execute(sql, (sql_title, sql_quantity, sql_quantityunit, sql_calorie, sql_cpf))
        conn.commit()
        conn.close()
        print(f'successful input of {sql_title} for {username}')
        await update.callback_query.answer()
...
if __name__ == '__main__':
    ...
    # now = datetime.datetime.now()
    # today2357 = now.replace(hour=13, minute=6, second=0, microsecond=0)
    #
    # #the purpose of this check is so that the script can be edited and run normally except between 2357-0000
    # #between these times, you cannot change the script until the autolog_scheduler while loop has finished running i.e. at 2358
    # if now>today2357: #if time is between 2357 and 0000 (i.e. when daily function is run through launchctl)
    #     autolog_scheduler() #start the autolog_scheduler (runs until 2358 then starts calling log_macros for each username)
